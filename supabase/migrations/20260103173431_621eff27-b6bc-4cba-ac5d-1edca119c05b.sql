...
BEGIN
  ...
  IF p_payment_method NOT IN ('razorpay', 'cod', 'upi', 'subscription_auto') THEN
    RAISE EXCEPTION 'Invalid payment method: %. Allowed values are: razorpay, cod, upi, subscription_auto', p_payment_method;
  END IF;

  -- Get order with backwards-compatible agent check
  SELECT * INTO v_order
  FROM orders
  WHERE id = p_order_id
  ...
  -- Calculate distance and payout (Zepto formula)
  v_distance := COALESCE(v_order.distance_km, 2);
  v_payout := v_base_pay + (v_distance * v_per_km_rate);

  -- Handle COD - use orders.total (correct column name)
  ...
  -- Insert delivery history (use orders.total for total_amount, orders.address for delivery_address)
