-- Fix customer name retrieval in complete_delivery_zepto
-- The function was using address->>'name' which may not exist
-- Update to use customer_name field from orders table with fallback to address name
...
BEGIN
  ...
  -- 8. Get customer name: Try customer_name field first, then address->>'name', then fallback
  v_customer_name := COALESCE(
    NULLIF(TRIM(v_order.customer_name), ''),
    NULLIF(TRIM(v_order.address->>'name'), ''),
    'Customer'
  );
  
  RAISE LOG 'complete_delivery_zepto: Using customer_name=% for order %', v_customer_name, p_order_id;

  -- 9. Insert into delivery_history (unique constraint will prevent duplicates)
  BEGIN
    ...
    RAISE LOG 'complete_delivery_zepto: Inserted delivery_history for order %, agent %, payout %, customer=%', 
      p_order_id, p_agent_id, v_payout, v_customer_name;
  EXCEPTION
  ...
  RAISE LOG 'complete_delivery_zepto: SUCCESS for order %, payout=%, distance=%km, customer=%', 
    p_order_id, v_payout, v_distance_km, v_customer_name;
...
-- Backfill existing delivery_history records with correct customer names from orders
UPDATE delivery_history dh
SET customer_name = COALESCE(
  NULLIF(TRIM(o.customer_name), ''),
  NULLIF(TRIM(o.address->>'name'), ''),
  dh.customer_name
)
FROM orders o
WHERE dh.order_id = o.id
  AND dh.customer_name = 'Customer'
  AND (
    NULLIF(TRIM(o.customer_name), '') IS NOT NULL 
    OR NULLIF(TRIM(o.address->>'name'), '') IS NOT NULL
  );