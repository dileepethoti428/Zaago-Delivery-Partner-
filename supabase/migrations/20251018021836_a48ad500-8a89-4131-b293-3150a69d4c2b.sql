-- Force refresh delivery completion functions to fix column reference issues

-- Drop all three completion functions with CASCADE
DROP FUNCTION IF EXISTS qr_complete_delivery_v3(UUID, TEXT, UUID) CASCADE;
DROP FUNCTION IF EXISTS manual_complete_delivery(UUID, TEXT, UUID) CASCADE;
DROP FUNCTION IF EXISTS simple_mark_delivered(UUID, UUID) CASCADE;

-- Recreate qr_complete_delivery_v3 with correct field references
...
BEGIN
  ...
  -- Insert into delivery_history using v_order.address
  INSERT INTO delivery_history (
    ...
    v_order.address,  -- Using address from orders table
    v_order.items,
    v_order.total,
    p_payment_method,
...
-- Recreate manual_complete_delivery with correct parameter order and field references
...
-- Recreate simple_mark_delivered with correct field references
...
GRANT EXECUTE ON FUNCTION qr_complete_delivery_v3(UUID, TEXT, UUID) TO authenticated;
