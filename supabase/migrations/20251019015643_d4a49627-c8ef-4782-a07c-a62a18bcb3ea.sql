-- Fix race condition in safe_complete_delivery by explicitly handling unique violations
...
BEGIN
  ...
  -- Wrap all inserts in a block to catch unique violations from race conditions
  BEGIN
    -- Update order status
    UPDATE orders
  ...
  EXCEPTION 
    WHEN unique_violation THEN
      -- If we get a unique violation, it means another concurrent request
      -- already completed this delivery. Return success since the goal is achieved.
      RETURN jsonb_build_object(
        'success', true,
        'message', 'Delivery completed by concurrent request',
        'order_id', p_order_id,
