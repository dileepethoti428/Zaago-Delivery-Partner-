-- Fix for delivery completion: normalize existing data first, then update functions

-- Step 1: Normalize existing payment_method values in delivery_history
UPDATE delivery_history
SET payment_method = CASE 
  WHEN UPPER(TRIM(payment_method)) = 'COD' THEN 'COD'
  WHEN UPPER(TRIM(payment_method)) IN ('ONLINE', 'ONLINE PAYMENT', 'UPI', 'CARD') THEN 'ONLINE'
  ELSE 'ONLINE'
END
WHERE payment_method IS NOT NULL;

-- Step 2: Now add the CHECK constraint (accepts both uppercase versions)
...
-- Step 3: Update qr_complete_delivery_v3 with normalized payment method
...
BEGIN
  -- Normalize payment method: convert to uppercase and map to COD or ONLINE
  v_normalized_payment := CASE 
    WHEN UPPER(TRIM(p_payment_method)) = 'COD' THEN 'COD'
    ELSE 'ONLINE'
  END;
  ...
  VALUES ('qr-completion@system.com', 'email_sent', jsonb_build_object(
    ...
    'order_id', p_order_id,
    'agent_id', p_agent_id,
    'original_payment_method', p_payment_method,
    'normalized_payment_method', v_normalized_payment,
    'timestamp', NOW()
  ));
  ...
  BEGIN
    ...
    -- Create delivery history with ALL fields (use UPSERT)
...
-- Step 4: Update nuclear bypass with same logic
...
COMMENT ON FUNCTION public.qr_complete_delivery_v3 IS 'QR-based delivery completion with normalized payment methods (COD or ONLINE only)';
COMMENT ON FUNCTION public.nuclear_complete_delivery_bypass IS 'Nuclear bypass for delivery completion with normalized payment methods (COD or ONLINE only)';