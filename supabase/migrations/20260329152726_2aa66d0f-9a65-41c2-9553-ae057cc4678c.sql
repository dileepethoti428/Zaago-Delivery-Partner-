...
-- First check if there's a unique constraint on (order_id, agent_id) for agent_earnings_tracking
-- If not, we need to create one for ON CONFLICT to work
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conrelid = 'public.agent_earnings_tracking'::regclass 
    AND contype = 'u'
    AND array_length(conkey, 1) = 2
  ) THEN
    -- Create unique constraint if it doesn't exist
    ALTER TABLE public.agent_earnings_tracking 
    ADD CONSTRAINT agent_earnings_tracking_order_agent_unique 
    UNIQUE (order_id, agent_id);
  END IF;
END $$;

-- Recreate complete_delivery_zepto with ON CONFLICT DO UPDATE for agent_earnings_tracking
...
BEGIN
  ...
  IF v_rows_affected = 0 THEN
    RETURN jsonb_build_object('success', true, 'already_completed', true, 'message', 'Delivery already completed (concurrent)');
  END IF;
...
-- Backfill: Fix all existing stuck pending records that were already delivered
