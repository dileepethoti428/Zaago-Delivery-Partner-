...
DROP INDEX IF EXISTS unique_agent_wallet_transaction_delivery;

-- Remove duplicate wallet transactions, keeping only the earliest
WITH duplicates AS (
  SELECT id, 
    ROW_NUMBER() OVER (
...
-- Update the function with proper conflict handling
...
BEGIN
  ...
  -- Update order
  UPDATE orders
  SET status = 'delivered', payment_status = v_new_payment_status,
  ...
  -- Update wallet (always add to balance)
  ...
  -- Wallet transaction (idempotent with new unique index)
