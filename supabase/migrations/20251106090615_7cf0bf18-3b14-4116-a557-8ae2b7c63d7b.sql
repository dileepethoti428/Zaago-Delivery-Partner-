-- Fix distance_km field error in delivery completion functions
-- Drop and recreate functions to remove reference to non-existent v_order.distance_km field
...
CREATE FUNCTION public.manual_complete_delivery(
    p_order_id TEXT,
    p_agent_id UUID,
...
CREATE FUNCTION public.qr_complete_delivery_v3(
    p_qr_code_data TEXT,
    p_agent_id UUID,
    p_payment_method TEXT DEFAULT 'COD'
)
...
BEGIN
    ...
    FROM order_qr_codes
    WHERE qr_code_data = p_qr_code_data AND scanned = false
    LIMIT 1;

    IF v_order_id IS NULL THEN
        RETURN jsonb_build_object(
            'success', false,
            'error', 'Invalid or already scanned QR code'
        );
    END IF;
    ...
    SELECT * INTO v_order
    FROM orders
    WHERE id = v_order_id AND agent_id = p_agent_id;
    ...
    INSERT INTO delivery_history (
        ...
        v_order_id, p_agent_id, v_order.customer_name, v_order.customer_phone,
        v_order.pickup_address, v_order.delivery_address, v_distance_km, v_payout_amount,
        p_payment_method, v_payment_status
    );
    ...
    -- Create wallet transaction
    INSERT INTO agent_wallet_transactions (agent_id, amount, transaction_type, description)
    VALUES (p_agent_id, v_payout_amount, 'earning', 'Delivery payout for order ' || v_order_id);
    ...
    RETURN jsonb_build_object(
        'success', true,
        'message', 'Delivery completed successfully via QR',
        'payout_amount', v_payout_amount,
        'already_completed', false
    );
END;
...
CREATE FUNCTION public.simple_mark_delivered(
    p_order_id TEXT,
    p_agent_id UUID,
    p_payment_method TEXT DEFAULT 'COD'
)
