-- COMPREHENSIVE FIX: Final resolution for QR delivery completion issues
-- This migration creates a single atomic function to handle all delivery completion logic

-- Step 1: Drop problematic trigger and recreate the function with correct column references
...
BEGIN
  IF NEW.status = 'delivered' AND NEW.agent_id IS NOT NULL THEN
    ...
    SELECT jsonb_build_object(
      'latitude', latitude,
      'longitude', longitude
    ) INTO agent_location_data
    FROM delivery_agents
    WHERE id = NEW.agent_id;
    
    -- Derive payment_method from payment_status (same logic as other triggers)
    ...
    -- Insert into delivery_history with correct column mappings
    INSERT INTO delivery_history (
      ...
      special_instructions, delivery_notes, agent_location,
      delivery_proof, customer_rating, distance_traveled,
      delivery_duration, delivery_payout
    ) VALUES (
      NEW.id, 
      NEW.agent_id,
      COALESCE((NEW.address->>'fullName'), (NEW.address->>'name'), 'Unknown'),
      COALESCE((NEW.address->>'phone'), ''),
      NEW.address, -- Using address column, not delivery_address
      NEW.items, 
      NEW.total, 
      NEW.payment_status, 
      derived_payment_method, -- Derived from payment_status
      CURRENT_DATE, 
      NEW.delivery_time_slot, 
      NEW.special_instructions,
      '', -- Empty string for delivery_notes (column doesn't exist in orders)
      agent_location_data, 
      '{}'::jsonb, -- Empty JSONB for delivery_proof (column doesn't exist in orders)
      NULL, 
      NULL, 
      NULL, 
      NULL
...
-- Step 2: Create atomic QR delivery completion function
...
BEGIN
  ...
  -- Get agent location
  SELECT jsonb_build_object('latitude', latitude, 'longitude', longitude)
  INTO v_agent_location
  FROM delivery_agents
  WHERE id = p_agent_id;
