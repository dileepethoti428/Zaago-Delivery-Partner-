-- Step 1: Remove duplicate unique constraint on delivery_history
ALTER TABLE delivery_history DROP CONSTRAINT IF EXISTS unique_order_agent_history;

-- Step 2: Update qr_complete_delivery_v2 to use explicit constraint name
...
BEGIN
  ...
  -- 5. Create delivery history record with explicit constraint reference (FIXED)
...
COMMENT ON FUNCTION qr_complete_delivery_v2 IS 'Complete QR delivery with idempotency using explicit constraint reference';