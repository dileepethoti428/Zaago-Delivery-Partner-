-- Explicitly set completed_at in all delivery completion functions
...
-- Recreate simple_mark_delivered with explicit completed_at
...
BEGIN
  ...
  INSERT INTO delivery_history (
    ...
    items, total_amount, delivery_date, payment_method, payment_status, delivery_payout, completed_at
  ) VALUES (
    p_order_id, p_agent_id, v_order.customer_name, v_order.customer_phone, v_order.address,
    v_order.items, v_order.total, CURRENT_DATE, 'COD', 'pending', 25.00, NOW()
  )
...
-- Recreate manual_complete_delivery with explicit completed_at
...
BEGIN
  ...
  INSERT INTO delivery_history (
    ...
    CASE WHEN v_normalized_payment = 'ONLINE' THEN 'paid' ELSE 'pending' END, 25.00, NOW()
  )
  ON CONFLICT (order_id, agent_id) DO NOTHING;
...
-- Recreate qr_complete_delivery_v3 with explicit completed_at
