-- Remove all existing overloads to fix PGRST203 once and for all
DO $$
DECLARE r record;
BEGIN
  FOR r IN
    ...
    WHERE n.nspname = 'public'
      AND p.proname IN ('manual_complete_delivery', 'simple_mark_delivered')
  LOOP
    EXECUTE format('DROP FUNCTION IF EXISTS %s;', r.funcsig);
  END LOOP;
END $$;

-- Recreate a single canonical version with stable search_path
...
BEGIN
  ...
  IF v_order.status = 'delivered' THEN
    RETURN jsonb_build_object('success', true, 'already_completed', true, 'payout_amount', 30, 'distance_km', 0);
  END IF;

  SELECT * INTO v_agent FROM delivery_agents WHERE id = p_agent_id;

  v_payment_method := CASE WHEN UPPER(TRIM(p_payment_method)) IN ('COD','CASH') THEN 'COD' ELSE 'ONLINE' END;
  v_payment_status := CASE WHEN v_payment_method = 'COD' THEN 'pending' ELSE 'paid' END;
  ...
  UPDATE orders SET status = 'delivered', delivered = true, delivered_at = NOW() WHERE id = p_order_id;
  ...
  RETURN jsonb_build_object('success', true, 'payout_amount', v_payout, 'distance_km', v_distance_km, 'already_completed', false);
