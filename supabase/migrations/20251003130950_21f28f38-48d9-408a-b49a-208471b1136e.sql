-- Create qr_complete_delivery_v3 with bulletproof idempotency
...
BEGIN
  ...
  -- Check if already delivered via order status (secondary check)
  ...
  -- STEP 2: Create delivery_history FIRST (acts as completion lock)
  ...
  -- STEP 4: Create earnings record (idempotent)
  ...
  -- STEP 5: Update agent wallet (idempotent)
  ...
  -- STEP 6: Create wallet transaction (with conflict handling)
...
COMMENT ON FUNCTION qr_complete_delivery_v3 IS 'Complete QR delivery with bulletproof idempotency - checks delivery_history first, inserts delivery_history as completion lock, all operations have conflict handling';

-- Ensure agent_wallet_transactions has proper unique constraint for idempotency
CREATE UNIQUE INDEX IF NOT EXISTS idx_agent_wallet_transactions_agent_order 
ON agent_wallet_transactions(agent_id, order_id) 
WHERE transaction_type = 'delivery_payment';