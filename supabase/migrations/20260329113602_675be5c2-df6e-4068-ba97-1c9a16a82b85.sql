CREATE OR REPLACE FUNCTION public.complete_delivery_zepto(p_order_id uuid, p_payment_method text, p_live_distance_km numeric DEFAULT NULL::numeric)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT id INTO v_da_id FROM delivery_agents WHERE agent_id = v_agent_id;
  IF v_da_id IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'Agent profile not found');
  END IF;

  SELECT id INTO v_existing_history FROM delivery_history WHERE order_id = p_order_id AND agent_id = v_da_id;
  IF v_existing_history IS NOT NULL THEN
    RETURN jsonb_build_object('success', true, 'message', 'Delivery already completed', 'idempotent', true);
  END IF;
  ...
  INSERT INTO delivery_history (
    ...
    payment_status, delivery_payout, distance_traveled, tip_amount,
  ...
  UPDATE orders SET status = 'delivered', updated_at = NOW(), delivered_at = NOW() WHERE id = p_order_id;
  ...
  INSERT INTO earnings (agent_id, order_id, amount, distance_km, payment_method, tip_amount, status)
  ...
  VALUES (v_da_id, v_payout, v_cod_amount, CASE WHEN v_normalized_payment = 'cod' THEN v_cod_amount ELSE 0 END)
  ON CONFLICT (agent_id) DO UPDATE SET
    balance = agent_wallet.balance + v_payout,
    total_collected = agent_wallet.total_collected + v_cod_amount,
    pending_cod_amount = CASE WHEN v_normalized_payment = 'cod' THEN agent_wallet.pending_cod_amount + v_cod_amount ELSE agent_wallet.pending_cod_amount END,
    updated_at = NOW();

  INSERT INTO agent_wallet_transactions (agent_id, order_id, amount, transaction_type, status, description)
  VALUES (v_da_id, p_order_id, v_payout, 'earning', 'completed', 'Delivery payout for order');

  UPDATE agent_earnings_tracking 
  SET completed_at = NOW(), actual_payout = v_payout, distance_km = v_distance_km,
      payment_method = v_normalized_payment, payout_status = 'completed', tip_amount = v_tip
  WHERE order_id = p_order_id AND agent_id = v_da_id;

  RETURN jsonb_build_object('success', true, 'payout', v_payout, 'distance_km', v_distance_km,
    'payment_method', v_normalized_payment, 'cod_amount', v_cod_amount, 'tip_amount', v_tip);
