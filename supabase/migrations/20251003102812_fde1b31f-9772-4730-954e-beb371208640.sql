-- Make qr_complete_delivery_atomic truly idempotent
-- Handle case where order is already delivered without throwing error
...
BEGIN
  ...
  -- The trigger will handle delivery_history insertion with ON CONFLICT
...
EXCEPTION WHEN OTHERS THEN
  -- Log error but check if it's just a duplicate
  IF SQLERRM LIKE '%unique_order_delivery%' THEN
    -- This means the order was already delivered, treat as success
    RETURN jsonb_build_object(
      'success', true,
