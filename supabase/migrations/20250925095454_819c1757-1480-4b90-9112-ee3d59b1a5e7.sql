-- Fix orders table schema and clean up corrupted data (without concurrent indexes)
...
AND (
  ...
  OR length(pickup_address) < 10
  OR pickup_address !~ '^[\s]*[{[].*[}\]][\s]*$'
);

-- Clean up any other problematic text fields that might contain invalid data
UPDATE orders 
...
-- Add helpful indexes for performance (without CONCURRENTLY)
CREATE INDEX IF NOT EXISTS idx_orders_agent_id_status ON orders(agent_id, status) WHERE agent_id IS NOT NULL;
