-- Fix delivery_history duplicate key handling in safe_complete_delivery
...
BEGIN
  ...
  -- This ensures only ONE request can process this order at a time
  SELECT * INTO v_order 
  FROM orders 
  WHERE id = p_order_id
  FOR UPDATE NOWAIT;
  ...
  -- Create delivery history record with better duplicate handling
  BEGIN
  ...
  EXCEPTION
    WHEN unique_violation THEN
      -- Delivery history already exists, this is fine
      NULL;
  END;
...
EXCEPTION 
  WHEN lock_not_available THEN
    -- Another request is currently processing this order, return success
    RETURN jsonb_build_object(
      'success', true,
